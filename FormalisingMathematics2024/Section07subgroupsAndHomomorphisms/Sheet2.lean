...
theorem comp_id : φ.comp (MonoidHom.id G) = φ := rfl


theorem id_comp : (MonoidHom.id H).comp φ = φ := rfl
...
example (φ : G →* H) (ψ : H →* K) (U : Subgroup K) : U.comap (ψ.comp φ) = (U.comap ψ).comap φ := rfl
...
example (φ : G →* H) (ψ : H →* K) (S : Subgroup G) : S.map (ψ.comp φ) = (S.map φ).map ψ := by
  ...
  constructor
  · intro hkS
    cases' hkS with g hgg
    obtain ⟨h1, h2⟩ := hgg
    rw [← h2]
    refine' ⟨φ g, _, rfl⟩
    exact ⟨g, h1, rfl⟩
  · rintro ⟨h, ⟨s, hs, rfl⟩, rfl⟩
    exact ⟨s, hs, rfl⟩
